...
namespace Core.WebAPI.ClientHelper
{
    public class APIClientHelper
    {
        ...
        public InsuranceServiceProvidersResponse GetInsuranceServiceProviders(string pstrToken, string UserName, string PageName)
        {
            InsuranceServiceProvidersResponse lobjServiceProvidersResponse = null;
            try
            {
                string APIResponse = string.Empty;
                if (PageName.Contains("Internet"))
                {
                    APIResponse = WebAPIHelper.PostData(APIConstant.GetInternetServiceProvider, "GET", "GetInternetServiceProviders", string.Empty, UserName, pstrToken);
                }
                else
                {
                    APIResponse = WebAPIHelper.PostData(APIConstant.GetInsuranceServiceProviders, "GET", "GetInsuranceServiceProviders", string.Empty, UserName, pstrToken);
                }

                if (APIResponse.Length > 0)
                {
                    lobjServiceProvidersResponse = JsonConvert.DeserializeObject<InsuranceServiceProvidersResponse>(APIResponse);
                }
                else
                {
                    lobjServiceProvidersResponse = null;
                }
            }
            catch (Exception ex)
            {
                LoggingAdapter.WriteLog("APIClientHelper GetInsuranceServiceProviders Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjServiceProvidersResponse = null;
            }
            return lobjServiceProvidersResponse;

        }

        public InsuranceRequiredDetailsResponse GetRequiredDetails(int ServiceCode, string UserName, string Token, string PageName)
        {
            InsuranceRequiredDetailsResponse lobjRequiredDetailsResponse = null;
            try
            {
                string APIResponse = string.Empty;
                if (PageName.Contains("ISPListDetails"))
                {
                    APIResponse = WebAPIHelper.PostData(APIConstant.GetISPRequiredDetails + "?code=" + ServiceCode, "GET", "GetRequiredDetails", string.Empty, UserName, Token);
                }
                else
                {
                    APIResponse = WebAPIHelper.PostData(APIConstant.GetRequiredDetails + "?code=" + ServiceCode, "GET", "GetRequiredDetails", string.Empty, UserName, Token);
                }

                if (APIResponse.Length > 0)
                {
                    lobjRequiredDetailsResponse = JsonConvert.DeserializeObject<InsuranceRequiredDetailsResponse>(APIResponse);
                }
                else
                {
                    lobjRequiredDetailsResponse = null;
                }
            }
            catch (Exception ex)
            {
            ...
            return lobjRequiredDetailsResponse;
        }

        public InsuranceUserDetailsResponse GetUserDetails(int code, string Policyno, string DOB, string UserName, string Token, string MembershipReference)
        {
            InsuranceUserDetailsResponse lobjUserDetailsResponse = null;
            try
            {
                UserDetailsParameters pobjFetchUserDetails = new UserDetailsParameters();
                if (DOB == "undefined")
                {
                    pobjFetchUserDetails = new UserDetailsParameters
                    {
                        ServiceCode = code,
                        RequestId = Policyno,
                ...
                string APIResponse = WebAPIHelper.PostData(APIConstant.FetchUserDetails, "POST", "FetchUserDetails", JsonConvert.SerializeObject(pobjFetchUserDetails), UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjUserDetailsResponse = JsonConvert.DeserializeObject<InsuranceUserDetailsResponse>(APIResponse);
                }
                else
                {
                    lobjUserDetailsResponse = null;
                }
            }
            catch (Exception ex)
            {
                LoggingAdapter.WriteLog("APIClientHelper GetUserDetails Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjUserDetailsResponse = null;
            }
            return lobjUserDetailsResponse;
        }

        public InsurancePaymentRequestResponse InsurancePaymentRequest(string code, decimal Amount, int SessionId, string PolicyNo, string TransactionId, string UserName, string Token, string ServiceName, string MembershipReference, string CustomerName)
        {
            InsurancePaymentRequestResponse lobjPaymentResponse = null;
            try
            {
                PaymentRequestParameters pobjPaymentRequest = new PaymentRequestParameters();
                switch (ServiceName)
                {
                    case "Nepal Insurance":
                    case "Himalayan Insurance":
                    case "Surya Life Insurance":
                        pobjPaymentRequest = new PaymentRequestParameters
                        {
                            ServiceCode = code,
                            Amount = Amount,
                            SessionId = SessionId,
                ...
                string APIResponse = WebAPIHelper.PostData(APIConstant.InsurancePaymentRequest, "POST", "InsurancePaymentRequest", JsonConvert.SerializeObject(pobjPaymentRequest), UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjPaymentResponse = JsonConvert.DeserializeObject<InsurancePaymentRequestResponse>(APIResponse);
                }
                else
                {
            ...
            {
                LoggingAdapter.WriteLog("APIClientHelper InsurancePaymentRequest Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjPaymentResponse = null;
            }
            return lobjPaymentResponse;

        }

        public List<InsuranceBookingResponse> GetBookedInsuranceListForMember(string pstrMemberId)
        {
            InsuranceBookingDetails lobjResponse = new InsuranceBookingDetails();
            List<InsuranceBookingResponse> lobjFinalResponse = new List<InsuranceBookingResponse>();
            try
            {
                ...
                dynamicCls.MemberId = pstrMemberId;
                ...
                string Parameters = JsonConvert.SerializeObject(lobjDictionary);
                string APIResponse = WebAPIHelper.PostData(APIConstant.GetBookedInsuranceListForMember, "POST", "GetBookedInsuranceListForMember", Parameters, UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjResponse = JsonConvert.DeserializeObject<InsuranceBookingDetails>(APIResponse);
                    lobjFinalResponse = lobjResponse.results;
                }
                else
                {
                    lobjResponse = null;
            ...
            {
                LoggingAdapter.WriteLog("APIClientHelper GetBookedInsuranceListForMember Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjResponse = null;
            }
            return lobjFinalResponse;
        }
        #endregion
        ...
        #region Khalti ISP
        public ISPUserDetailsResponse GetISPUserDetails(int code, string UserId, string ServiceName, string UserName, string Token, string MembershipReference)
        {
            ISPUserDetailsResponse lobjUserDetailsResponse = null;
            try
            {
                UserDetailsParameters pobjFetchUserDetails = new UserDetailsParameters();
                switch (ServiceName)
                ...
                string APIResponse = WebAPIHelper.PostData(APIConstant.FetchISPUserDetails, "POST", "FetchUserDetails", JsonConvert.SerializeObject(pobjFetchUserDetails), UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjUserDetailsResponse = JsonConvert.DeserializeObject<ISPUserDetailsResponse>(APIResponse);
                }
                else
                {
                    lobjUserDetailsResponse = null;
            ...
            {
                LoggingAdapter.WriteLog("APIClientHelper GetISPUserDetails Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjUserDetailsResponse = null;
            }
            return lobjUserDetailsResponse;
        }

        public GetDiscountResponse GetDiscount(GetDiscountRequest lobjRequest)
        {
            ...
            try
            {
                ...
                string APIResponse = WebAPIHelper.PostData(APIConstant.GetDiscount, "POST", "GetDiscount", JsonConvert.SerializeObject(lobjRequest), UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjGetDiscountResponse = JsonConvert.DeserializeObject<GetDiscountResponse>(APIResponse);
                }
                else
                {
                    lobjGetDiscountResponse = null;
                }
            }
            catch (Exception ex)
            {
                LoggingAdapter.WriteLog("APIClientHelper GetDiscount Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjGetDiscountResponse = null;
            }
            return lobjGetDiscountResponse;
        }

        public ISPPaymentResponse ISPPaymentRequest(ISPPaymentRequest lobjRequest)
        {
            ISPPaymentResponse lobjISPPaymentResponse = new ISPPaymentResponse();
            try
            {

                string Token = Convert.ToString(ConfigurationManager.AppSettings["KhaltiISPToken"]);
                string UserName = Convert.ToString(ConfigurationManager.AppSettings["KhaltiISPUserName"]);
                string APIResponse = WebAPIHelper.PostData(APIConstant.ISPPaymentRequest, "POST", "ISPPaymentRequest", JsonConvert.SerializeObject(lobjRequest), UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjISPPaymentResponse = JsonConvert.DeserializeObject<ISPPaymentResponse>(APIResponse);
                }
                else
                {
                    lobjISPPaymentResponse = null;
                }
            }
            catch (Exception ex)
            {
                LoggingAdapter.WriteLog("APIClientHelper ISPPaymentRequest Exception: " + ex.Message + Environment.NewLine + ex.StackTrace);
                lobjISPPaymentResponse = null;
            }
            return lobjISPPaymentResponse;
        }

        public List<ISPBookingResponse> GetBookedISPListForMember(string pstrMemberId)
        {
            ISPBookingDetails lobjResponse = new ISPBookingDetails();
            List<ISPBookingResponse> lobjFinalResponse = new List<ISPBookingResponse>();
            try
            {
                string Token = Convert.ToString(ConfigurationManager.AppSettings["KhaltiISPToken"]);
                string UserName = Convert.ToString(ConfigurationManager.AppSettings["KhaltiISPUserName"]);
                ...
                string Parameters = JsonConvert.SerializeObject(lobjDictionary);
                string APIResponse = WebAPIHelper.PostData(APIConstant.GetBookedISPListForMember, "POST", "GetBookedISPListForMember", Parameters, UserName, Token);
                if (APIResponse.Length > 0)
                {
                    lobjResponse = JsonConvert.DeserializeObject<ISPBookingDetails>(APIResponse);
                    lobjFinalResponse = lobjResponse.results;
                }
                else
                {
