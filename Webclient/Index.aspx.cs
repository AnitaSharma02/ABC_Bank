                ...
                if (listOfCategories != null && listOfCategories.Count > 0)
                {
                    ...
                    foreach (var category in listOfCategories.FindAll(lobj => lobj.ParentId.IsNullOrEmpty() && lobj.IsActive).OrderBy(o => o.Priorty).ToList())
                    {
                        try
                        {
                            ...
                            sb.Append("<div class=\"col-auto mb-md-3\"><a class=\"text-center d-flex flex-column align-items-center " + category.Name.Replace(" ", "").Replace("-", "").ToLower() + "redemption redemptionoptions\" href=\"" + PageURL + "\"><div class=\"d-flex flex-column align-items-center justify-content-center p-2 p-lg-2 p-xl-4 rounded-circle imageBox\"><img src=\"" + (category.PrimaryImage != null && category.PrimaryImage.Url != null ? category.PrimaryImage.Url : string.Empty) + "\" /></div><p class=\"mt-3\">" + category.Name + "</p></a></div>");
                        }
                        catch (Exception)
                        {
