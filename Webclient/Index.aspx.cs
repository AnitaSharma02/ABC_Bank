        ...
        try
        {
            if (string.IsNullOrEmpty(Convert.ToString(HttpContext.Current.Application[path])))
            {
                ...
                if (storeAssets != null)
                {
                    ...
                    sb.Append("</div>");
                    sb.Append("<div class=\"swiper-pagination container-lg p-0 text-lg-right\"></div>");
                ...
                if (listOfCategories != null && listOfCategories.Count > 0)
                {
                    ...
                    foreach (var category in listOfCategories.FindAll(lobj => lobj.ParentId.IsNullOrEmpty() && lobj.IsActive).OrderBy(o => o.Priorty).ToList())
                    {
                        try
                        {
                            ...
                            sb.Append("<div class=\"col-4 col-sm-2 col-lg-2 col-xl-1 mb-3\"><a class=\"text-center d-flex flex-column align-items-center " + category.Name.Replace(" ", "").Replace("-", "").ToLower() + "redemption redemptionoptions\" href=\"" + PageURL + "\"><div class=\"d-flex flex-column align-items-center justify-content-center p-2 p-lg-2 p-xl-4 rounded-circle imageBox\"><img src=\"" + (category.PrimaryImage != null && category.PrimaryImage.Url != null ? category.PrimaryImage.Url : string.Empty) + "\" /></div><p>" + category.Name + "</p></a></div>");
                        }
                        catch (Exception)
                        {
                                ...
                                if (offers[0].ToLower().Contains("giftcards"))
                                {
                                    ...
                                    if (searchResult != null && searchResult.Products != null && searchResult.Products.Count > 0)
                                    {
                                        ...
                                        {                                          
                                            ...
                                            sb.Append(string.Format("<div class=\"swiper-slide shadow-sm\"><a class=\" bg-colour6 d-block\" href=\"ProductDetails.aspx?ProductId={4}&ProductType={2}\"><div class=\"img-container\"><img src=\"{0}\"  alt=\"\" /></div><div class=\"bg-white d-flex flex-column p-2\"><p class=\"h6 heading-semibold text-colour7 text-truncate mb-2 w-100\">{1}</p><p class=\"h8 heading-regular\">{2}</p><p class=\"h7 heading-medium text-colour7 text-truncate pt-2\">{3}</p></div></a></div>", product.PrimaryImage.Url, product.Name, pstrProductType, lobjModel.FormatPoints(Math.Ceiling(product.Price.SalePriceWithTax.Amount), "Points"), product.Id));                                            
                                        }
