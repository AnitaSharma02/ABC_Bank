...
$(document).ready(function () {
    ...
    if (PageName == 'StatementSummary.aspx' || PageName == 'TransactionSummary.aspx' || PageName == 'PointsExpiry.aspx' || PageName == 'ManageBooking.aspx' || PageName == 'ViewMemberProfile.aspx' || PageName == 'PurchaseEarser.aspx' || PageName == 'PointsPurchase.aspx' || PageName == 'OrderHistory.aspx' || PageName == 'OrderHistory.aspx' || PageName == "OrderDetails.aspx") {
        ...
        // html += '<div class="col-6 col-lg-2"><a class="nav-link d-flex align-items-center justify-content-around text-center heading-semibold px-3 px-lg-4 border" id="divPointsPurchase" href="PointsPurchase.aspx" data-i18n="index-miles-txt-PointsPurchase"></a></div>';
        // html += '<div class="col-6 col-lg-2"><a class="nav-link d-flex align-items-center justify-content-around text-center heading-semibold px-3 px-lg-4 border" id="divPurchaseEraser" href="PurchaseEarser.aspx" data-i18n="text-cashback-deal"></a></div>';
...
function ActivationOTPValidation() {
    ...
    if (msg.length > 0) {
        ...
        var MemberId = $("#CP_txtMemberId").val();
        $.ajax({
            url: 'Activation.aspx/GenerateOTP',
            type: 'POST',
            contentType: 'application/json; charset=utf-8',
                ...
                try {
                    ...
                    if (newData != "") {
                        if (newData == "Success") {
                            ...
                            $("#CP_hfMemberId").val(MemberId);
                            $("#CP_lblMessagesDetails")[0].innerHTML = "Your OTP has been sent to your Mobile No & Email.";
                            $("#CP_ErrorMsgContainer").show();
                        }
                            ...
                            $("#CP_lblMessagesDetails")[0].innerHTML = "Invalid details or Member not registered.";
                            $("#CP_ErrorMsgContainer").show();
                        }
                        else if (newData == "OTPFAILED") {
                            $("#CP_lblMessagesDetails")[0].innerHTML = "Failed to generate OTP.";
                            $("#CP_ErrorMsgContainer").show();
                        }
                        else if (newData == "Already Activated") {

                            $("#CP_lblMessagesDetails")[0].innerHTML = "Account is already activated.";
                            $("#CP_ErrorMsgContainer").show();
                        }
...
function ActivationValidationCodeBehind() {
    ...
    if (msg.length > 0) {
        ...
        $('#CP_hfRIM').val($('#CP_txtERIM').val())
        $('#CP_hfQID').val($('#CP_txtEQID').val())
        return true;
    }
}
